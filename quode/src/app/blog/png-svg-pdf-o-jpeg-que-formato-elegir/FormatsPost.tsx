...
import { Image as ImageIcon, PenTool, FileText, Camera } from "lucide-react";
import LegalHeader from "@/components/LegalHeader";
import { useLocale } from "@/lib/i18n/LocaleContext";
import { getUi } from "@/lib/i18n/ui";
import { Blocks, renderInline } from "@/lib/i18n/articleBlocks";
import { FORMATS_POST_CONTENT } from "@/content/blog/formatosDescarga";

const FORMAT_ICONS = [ImageIcon, PenTool, FileText, Camera];

export default function FormatsPost() {
  const { locale } = useLocale();
  const t = getUi(locale);
  const c = FORMATS_POST_CONTENT[locale];
          ...
          <div className="my-8 flex flex-wrap justify-center gap-3">
            {c.formats.map((label, i) => {
              ...
              return (
                <div
                  key={label}
                  className="w-20 bg-page rounded-2xl border border-secondary px-4 py-3 flex flex-col items-center gap-1.5"
                >
